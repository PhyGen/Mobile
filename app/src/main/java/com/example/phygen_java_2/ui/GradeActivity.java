        ...
        apiService.getGrades().enqueue(new Callback<List<Grade>>() {
            ...
            public void onResponse(Call<List<Grade>> call, Response<List<Grade>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    GradeAdapter adapter = new GradeAdapter(response.body(), grade -> {
                        Intent intent = new Intent(GradeActivity.this, SemesterActivity.class);
                        intent.putExtra("grade", grade.getName());
